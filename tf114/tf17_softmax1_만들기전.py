...
#4. 평가, 예측
y_predict = tf.nn.softmax(tf.matmul(tf.cast(x_data, tf.float32), w_val) + b_val)

y_pred = sess.run(tf.argmax(y_predict, axis=1)) 
y_data_val = sess.run(tf.argmax(y_data, axis=1)) 

# 정확도 계산
from sklearn.metrics import accuracy_score
acc = accuracy_score(y_pred, y_data_val)  
