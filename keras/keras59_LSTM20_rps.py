...
from tensorflow.keras.layers import Dense, Conv2D, Flatten, Dropout, MaxPooling2D, BatchNormalization, LSTM, Bidirectional
import time
...
print(x_train.shape, y_train.shape)     # (12268, 100, 100, 3) (12268, 3)
print(np.unique(y_train, return_counts=True))   # (array([0., 1.], dtype=float32), array([24536, 12268], dtype=int64))

x_train = x_train.reshape(12268, 100*3, 100)
x_test = x_test.reshape(252, 100*3, 100)

#2. 모델 구성
model = Sequential()
model.add(Bidirectional(LSTM(32, input_shape=(300,100), activation='relu', return_sequences=True)))
...
path = './_save/keras59/8_rpg/'
filename = '{epoch:04d}-{val_loss:.4f}.hdf5' 
filepath = "".join([path, 'k59_08_', date, '_', filename])   
#####################################
