param_bounds = {'x1' : (-1, 5),     # 범위
                'x2' : (0, 4), }    # 범위

def y_function(x1, x2):
    return -x1 **2 -(x2 - 2) **2 + 10

# BayesianOptimization : 최대값/최소값 찾기 => 최적화된 매개변수/파라미터를 찾음

# pip install bayesian-optimization
from bayes_opt import BayesianOptimization

optimizer = BayesianOptimization(
    f = y_function,         # 해당 함수로 최대값을 반환해 달라
    pbounds=param_bounds,   # x1과 x2의 범위를 지정함
    random_state=333,
)

optimizer.maximize(init_points=5,   # 5번을 돎, 처음 탐색 횟수
                   n_iter= 20,      # 20번을 돌게 함, 연산 횟수 -> 총 25번
                   )
...
# =================================================
# {'target': np.float64(9.999999219357809), 'params': {'x1': np.float64(-0.000249975830308687), 'x2': np.float64(1.9991525601637667)}}
